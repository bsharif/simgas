...
import { SEVOFLURANE_MAC_CONCENTRATION } from '../../../engine/patient'

const MAC_PER_CONC = SEVOFLURANE_MAC_CONCENTRATION
...
export function numericValue(id: NumericId, state: PatientState): string {
  switch (id) {
    ...
    case 'mac':   return (state.sevoflurane / MAC_PER_CONC).toFixed(1)
